...
class NeuralNet:
    ...
    def compute_activations(self, x, target):
        ...
        for layer in self._layers:
            output = layer.compute_activation(output)          # output of the last layer        #output = layer.compute_activation(output)
    ...
    def compute_gradients(self):
        ...
        input_gradient = self.loss.get_input_error_gradient()
        for layer in reversed(self._layers):
            layer.set_output_error_gradient(input_gradient)
            layer.compute_gradient()
            input_gradient = layer.get_input_error_gradient()
