...
class DenseLayer(BaseLayer):
    ...
    def compute_activation(self, x):
        ...
        output =  x.dot(self.w) + self.b            #maybe right maybe not;
        
        #self.db = np.matrix([np.sum(output, axis=0)])
    ...
    def compute_gradient(self):
        ...
        self.dw = np.transpose(np.dot(np.transpose(dE), self._input_data))
        self.db = np.dot(dE, 1)
        self.db = np.sum(self.db)
        self._input_error_gradient = np.dot(dE, np.transpose(self.w))
    ...
    def update_weights(self, learning_rate):
        ...
        self.w = self.w - learning_rate*self.dw             #maybe right
        self.b = self.b - learning_rate*self.db             #maybe right
